...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        const formData = new FormData(contactForm);
        formData.append('action', 'dost_audit_contact_form');
        
        // Check if dostAuditData exists (localized script)
        const ajaxUrl = (typeof dostAuditData !== 'undefined') ? dostAuditData.ajaxUrl : '/wp-admin/admin-ajax.php';
        const nonce = (typeof dostAuditData !== 'undefined') ? dostAuditData.nonce : '';
        
        if (nonce) {
            formData.append('dost_audit_contact_nonce', nonce);
        }
